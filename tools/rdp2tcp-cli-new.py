...
class RDP2TCPEnhancedCLI:
    ...
    def shell_tunnel(self, local_port=0, command='cmd.exe', args=None, auto_connect=False) -> bool:
        """Create a shell tunnel and optionally connect to it"""
        try:
            if not self.connect():
                return False
            ...
            if auto_connect:
                ...
                self.logger.info("To connect manually, use: telnet 127.0.0.1 " + str(local_port))
                return True
...
Examples:
  ...
  rdp2tcp-cli sh --local-port 4444 --shell-command powershell.exe
    ...
    # Shell command
    shell_parser = subparsers.add_parser('sh', help='Open a shell tunnel')
